...
import type { ShaderPack } from "../../shared/types";
...
export function ShaderPackPanel({ packs, onToggle, onRemove, onRefresh }: Props) {
  return (
    <div className="glass-card relative z-10 w-[420px] max-h-[500px] rounded-2xl p-5">
      <div className="mb-4 flex items-center justify-between">
        <h2 className="text-base font-semibold">Shader Paketleri</h2>
      ...
      <p className="mb-3 text-[10px] text-text-dim/60">
        Shader paketlerini {`<gameDir>/shaderpacks`} klasörüne koyun. OptiFine veya Iris gerekli.
      </p>

      <div className="max-h-[340px] space-y-2 overflow-y-auto">
        {packs.length === 0 && (
          <div className="py-8 text-center text-sm text-text-dim">
            Shader paketi bulunamadı.
          </div>
        )}
        {packs.map((p) => (
          <div
          ...
          >
            <div className={`h-2.5 w-2.5 rounded-full ${p.enabled ? "bg-amber-400" : "bg-text-dim"}`} />
