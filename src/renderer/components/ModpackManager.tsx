...
import type { ModpackInfo } from "../../shared/types";

interface Props {
  modpacks: ModpackInfo[];
  onSearch: (query: string, source: "curseforge" | "modrinth") => void;
  onInstall: (modpack: ModpackInfo) => void;
...
export function ModpackManager({ modpacks, onSearch, onInstall, installing }: Props) {
  ...
  return (
    <div className="glass-card relative z-10 w-[420px] max-h-[500px] rounded-2xl p-5">
      <h2 className="mb-4 text-base font-semibold">Modpack Manager</h2>
      <div className="mb-3 flex gap-2">
        <input className="input-field flex-1" placeholder="Modpack ara..." value={query} onChange={(e) => setQuery(e.target.value)} onKeyDown={(e) => e.key === "Enter" && onSearch(query, source)} />
      ...
      <div className="max-h-[300px] space-y-2 overflow-y-auto">
        {modpacks.length === 0 && <div className="py-6 text-center text-sm text-text-dim">Modpack arayın.</div>}
        {modpacks.map((m) => (
          <div key={m.id} className="flex items-center gap-3 rounded-xl border border-border bg-surface/50 p-3 transition-colors hover:border-accent">
            ...
            <div className="min-w-0 flex-1">
              ...
              <div className="truncate text-[10px] text-text-dim">{m.author} • {formatDownloads(m.downloads)} indirme • {m.mcVersion}</div>
              <div className="truncate text-[10px] text-text-dim/60">{m.description}</div>
            </div>
            <button onClick={() => onInstall(m)} disabled={!!installing} className="play-button !h-8 rounded-lg px-3 text-[10px] font-semibold text-white disabled:opacity-50">
              {installing === m.id ? "..." : "Kur"}
            </button>
          </div>
        ))}
