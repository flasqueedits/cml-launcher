...
const api: Api = {
  ...
  listConfigFiles: (gameDir: string) => ipcRenderer.invoke("config:list", gameDir),
  readConfigFile: (filePath: string) => ipcRenderer.invoke("config:read", filePath),
  writeConfigFile: (filePath: string, content: string) => ipcRenderer.invoke("config:write", filePath, content),
  ...
  setActiveInstance: (id: string) => ipcRenderer.invoke("instances:set-active", id),
  checkModUpdates: (gameDir: string) => ipcRenderer.invoke("mod-updates:check", gameDir),
  installModUpdate: (gameDir: string, mod: { name: string; downloadUrl: string; source: string }) => ipcRenderer.invoke("mod-updates:install", gameDir, mod),
  backupAllWorlds: (gameDir: string) => ipcRenderer.invoke("worlds:backup-all", gameDir),
